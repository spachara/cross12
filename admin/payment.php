		...
		$paymentArr = explode(" | ",$paymentArr1);
					...
					if($_POST['tran'][$r] == '2'){
						...
						$order_status = 2;
                                                ...
                                                if(isset($_POST['order_acc'][$r]) && $_POST['order_acc'][$r] == '1'){
                                                    $gen_invoice =1;
                                                }                                                    
					}
					...
					if($gen_invoice == 1)
                                        {
                                            $sql_update .= ", order_acc = '1' , invoice_no =  (select invoice_no  from (select IFNULL(max(invoice_no),0)+1 as invoice_no from order_tb where order_acc = '1') as invoice_no )";
                                            $sql_update .= ",invoice_remark ='" . $_POST['tran_comment'][$r] . "'";
                                        }
...
$(function() {
   ...
   $( "select[name^='tran']" ).change(function() {
       if($( this ).val() == '1' || $( this ).val() == '3' )
        $( this ).parents("td:first").find("input[name^='tran_comment']").val($( this ).parents("td:first").find("input[name='hideComment']").val());
        else
        $( this ).parents("td:first").find("input[name^='tran_comment']").val('');    
    });
});
    ...
    <td bgcolor="#000000"><table width="100%" cellpadding="0" cellspacing="1" border="0">
      ...
      <?php 
		...
		foreach($arrCheck as $key => $e){
			...
			$sql_order_tb .= "where order_status <> 0 and order_number =  '".$aad[0]."-".$aad[1]."'";
      ...
      <tr>
        ...
        <td valign="top" bgcolor="#F5F5F5"><input type="hidden" name="hidden_id[<?php echo $id[$key];?>]" value="<?php echo $id[$key];?>" />
          ...
          บาท 
        <br />
        <input type="checkbox" value="1" name="order_acc[<?php echo $id[$key];?>]">Generate invoice
            <br />
          <input type="text"  name="tran_comment[<?php echo $id[$key];?>]" />
            <input type="hidden" name="hideComment"  value="<?php echo 'เงินโอน ' . number_format($total_price[$key],2) . ' บาท ' . $bank_tranfer[$key];?>" />
        </td>
